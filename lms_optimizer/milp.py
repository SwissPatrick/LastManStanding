...
def milp_optimize(entry_candidates: dict[str, list[str]], scenarios: list[dict[str, bool]], weights: PortfolioWeights | None = None, exposure_cap: int | dict[str, int] | None = None, soft_exposure_cap: int | dict[str, int] | None = None, soft_penalty: float | None = None, scenario_probabilities: list[float] | None = None, future_values: dict[str, dict[str, float]] | None = None) -> MILPResult:
    ...
    c=np.zeros(n); integrality=np.zeros(n); integrality[:n_x+n_s]=1; lower=np.zeros(n); upper=np.full(n, np.inf); upper[:n_x+n_s]=1; integrality[y_start:] = 1; upper[y_start:] = 1; lower[eta_i]=0; upper[eta_i]=len(entries)
    ...
    # z is the at-least-one-survivor indicator; wipeout is its complement.
    # Therefore +at_least_one*z - wipeout*(1-z) has a constant -wipeout.
    ...
    result=milp(c, integrality=integrality, bounds=Bounds(lower,upper), constraints=LinearConstraint(np.asarray(rows), np.asarray(lows), np.asarray(highs)), options={"time_limit":2})
    # A time-limited incumbent is still a valid MILP allocation.  Preserve
    # the solver message in the audit while allowing the historical evaluator
    # to use the feasible incumbent rather than silently falling back to greedy.
    if not result.success and (result.x is None or not np.all(np.isfinite(result.x))): return MILPResult({}, float("nan"), False, result.message, result)
